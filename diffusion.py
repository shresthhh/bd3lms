...
class Diffusion(L.LightningModule):
  ...
  def _ddpm_caching_update(self, x, t, dt, p_x0=None):
    ...
    if self.config.sampling.first_hitting:
      ...
      num_masked_list = num_masked.tolist()

      # Sample a masked index per example
      inds = [
          torch.randint(0, nm, (1,)).item() if nm > 0 else 0
          for nm in num_masked_list
      ]
      ind = torch.tensor(inds, device=x.device)

      masked_positions = (x[:, -self.block_size:] == self.mask_index).nonzero(as_tuple=False)
      ind = masked_positions[ind, 1]
  ...
  def _semi_ar_sampler(
    ...
    for stride_num in tqdm(range(num_strides)):
      ...
      for i in range(num_steps):
        ...
        if self.config.sampling.first_hitting:
          u = np.random.rand()
          num_masked = int((x_accum[:, fwd_idx] == self.mask_index).sum().item())
          if num_masked == 0 :
            break
          t *= u**(1 / num_masked)
